...
def equipments(request):
    ...
    department = sessions['selected_department'].upper()
    manufacturer = request.GET.get('manufacturer')
    model_number = request.GET.get('modelNumber')
    uid = request.GET.get('uid')
    conn = build_connection_with_database()
    cur = conn.cursor()
    query1 = f"INSERT INTO problems(department, manufacturer, model_no, uid) VALUES ('{department}', '{manufacturer}', '{model_number}', '{uid}')"
    query2 = f"UPDATE info SET \"Status\" = 'Not Working' WHERE \"Model Number\" = '{model_number}'"
    cur.execute(query1)
    cur.execute(query2)
    close_connection_with_database(cur, conn)
    # engineers_no = "+9165656565"
